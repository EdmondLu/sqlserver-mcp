...
- Added `read_lob` for exactly-one-row/one-column text and binary LOB reads. A single sequential scan incrementally hashes and measures the complete byte-bounded value while retaining only the requested chunk (unless bounded report inspection is explicit). Text keeps a cross-system UTF-8 content hash while resumption cursors separately bind the exact UTF-16LE code-unit identity; binary identity uses raw bytes. Cursors also bind SQL/parameters, kind, and lengths, and fail with `LOB_CURSOR_EXPIRED` instead of mixing changed source versions, including malformed UTF-16 sequences that share a replacement-fallback UTF-8 hash.
- Added offline `inspect_report_payload` and `compare_report_payloads` with SHA-256, Base64/GZip integrity, UTF-8 BOM, XML declaration, exact CRLF/LF/CR counts, Base64 canonical/whitespace properties, root, optional unique-node, and compressed/decompressed byte-diff metadata. Optional raw-fragment inputs prove an exact single byte replacement; these tools never reserialize XML and explicitly do not claim FastReport loading or rendering validation.
- Added offline `replace_report_payload_fragment` to build the candidate through one unique direct replacement in the original decompressed bytes, then emit a new canonical Base64/GZip value, all layer hashes, invariants, exact proof, output-policy metadata, and guarded-patch payload arguments. An optional complete `patchTarget` makes the returned `nextRequest.arguments` directly callable as `generate_guarded_report_patch`; otherwise the response explicitly lists missing target fields. It never connects to SQL Server or reserializes XML, and distinguishes decompressed-byte preservation from non-guaranteed compressed-stream/GZip-header preservation.
- Added `generate_guarded_report_patch`, which only returns SQL text and never opens a database connection. Generation requires an exact single raw-byte replacement proof plus preserved invariants and produces an embedded preimage and hash/structure guards. `@Apply=0` is a read-only preflight that returns before transactions, `UPDLOCK`/`HOLDLOCK`, or `UPDATE`; only `@Apply=1` locks, revalidates, updates, verifies, and commits. No writable MCP tool was introduced.
- Added structured read-only Guard rejection details and common password/token redaction for returned and logged errors. Added bounded `limits.maxLobMb` and `limits.maxLobChunkSize` defaults.
